...
object FormVerifyUtils {
    ...
    private fun input(formItem: FormItem): Verify {
        ...
        val nullValue = formItem.form_item_value?.values == null
                || formItem.form_item_value?.values!!.isEmpty()
        if (required && nullValue) {
            /*必填，未填*/
            return Verify(false, "请填写【${formItem.form_item_key}】")
        } else if (required && !nullValue) {
            /*必填，已填，判断文字是否为空*/
            if (TextUtils.isEmpty(formItem.form_item_value?.values!![0].input_value)) {
                return Verify(false, "请填写【${formItem.form_item_key}】")
            }
        }
        if (!nullValue) {
    ...
    private fun chose(formItem: FormItem): Verify {
        ...
        if (required && nullValue) {
            ...
            } else if (formItem.form_item_value!!.limit_max != null && formItem.form_item_value!!.limit_max!! > 1) {
                /*必填，多选，判断是否选择*/
                var chose = 0
                for (value in formItem.form_item_value?.values!!) {
                    if (value.ck_check) {
